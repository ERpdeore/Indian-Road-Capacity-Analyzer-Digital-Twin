...
/* ============================================================
   DIGITAL TWIN DASHBOARD — JavaScript
   Add this entire block to the bottom of static/app.js
   ============================================================ */
...
/* ================================================================
   Entry point — called right after a successful image analysis
   (drop into the existing result-rendering code in app.js)

   Usage inside your existing renderResult() or handleSuccess():
       if (data.digital_twin_status === 'running') {
           dtShowPanel();
           dtStartPolling();
       }
...
async function dtPoll() {
  try {
    ...
    if (data.status === 'done' && data.twin_data) {
      ...
      console.error('[DT] MATLAB error:', data.error);
    } else {
      dtSetBadge('running', 'Simulink running…');
    }
  } catch (e) {
    console.warn('[DT] Poll failed:', e);
  }
}

/* ================================================================
   Render the twin data from MATLAB into the dashboard
   ================================================================ */
function dtRender(twin) {
  const s = twin.summary;

  /* ---- Scalar stats ---- */
  setText('dt-ideal-cap',    Math.round(s.ideal_capacity_pcu_hr));
  setText('dt-defect-cap',   Math.round(s.defect_capacity_pcu_hr));
  setText('dt-ideal-vol',    Math.round(s.ideal_volume_design_pcu));
  setText('dt-defect-vol',   Math.round(s.defect_volume_design_pcu));
  setText('dt-defect-speed', s.steady_state_speed_kmh.toFixed(1));
  setText('dt-defect-los',   s.level_of_service);
  setText('dt-loss-pct',     s.capacity_loss_pct.toFixed(1) + '%');

  /* ---- LOS meter ---- */
  document.querySelectorAll('.dt-los-seg').forEach(seg => {
    seg.classList.toggle('active', seg.dataset.los === s.level_of_service);
  });

  /* ---- Speed section ---- */
  if (s.pothole_speed_impact_pct > 0) {
    document.getElementById('dt-speed-section').style.display = '';
    setText('dt-speed-reduction', s.speed_reduction_pct.toFixed(1));
    setText('dt-congested-speed', s.steady_state_speed_kmh.toFixed(1));
  }
  ...
  if (phEl && s.pothole_speed_impact_pct > 0) phEl.style.display = '';
}

/* ================================================================
   Canvas Charts  (no external library — pure Canvas 2D API)
   ================================================================ */
function dtDrawCapChart(twin) {
  const canvas = document.getElementById('dt-cap-chart');
  if (!canvas) return;
  const ctx = canvas.getContext('2d');
  const t   = twin.simulation_time_s;
  const ideal  = twin.ideal_capacity_pcu_hr;
  const defect = twin.defect_capacity_pcu_hr;

  dtDrawLineChart(ctx, canvas, t, [
    { data: ideal,  color: '#22c55e', label: 'Ideal' },
    { data: defect, color: '#ef4444', label: 'Defect' },
  ]);
}
...
function dtDrawSpdChart(twin) {
  ...
  const t   = twin.simulation_time_s;
  dtDrawLineChart(ctx, canvas, t, [
    { data: twin.vehicle_speed_kmh, color: '#f97316', label: 'Speed km/h' },
  ]);
}
...
function dtDrawLineChart(ctx, canvas, xData, series) {
  ...
  [0, 15, 30, 45, 60].forEach(sec => {
    ctx.fillText(sec + 's', xScale(sec), PAD.top + plotH + 16);
  });
  ...
  series.forEach(({ data, color }) => {
    ...
    ctx.beginPath();
    data.forEach((v, i) => {
      const x = xScale(xData[i]), y = yScale(v);
    ...
    ctx.lineTo(xScale(xData[xData.length-1]), yScale(yMin));
    ctx.lineTo(xScale(xData[0]), yScale(yMin));
...
function dtAnimateRoads(summary) {
  if (DT.animTimer) clearInterval(DT.animTimer);

  /* spawn vehicles proportional to volume */
  const idealCount  = Math.max(1, Math.round(summary.ideal_volume_design_pcu  / 300));
  const defectCount = Math.max(1, Math.round(summary.defect_volume_design_pcu / 300));
  const idealSpeed  = 50;
  const defectSpeed = summary.steady_state_speed_kmh;

  spawnVehicles('dt-ideal-vehicles',  idealCount,  idealSpeed,  '#22c55e');
  spawnVehicles('dt-defect-vehicles', defectCount, defectSpeed, '#ef4444');
}
...
function spawnVehicles(containerId, count, speedKmh, accentColor) {
  ...
  const durBase = Math.max(1.5, 50 / speedKmh * 3);  // slower vehicles = longer animation
  ...
  for (let i = 0; i < Math.min(count, 6); i++) {
    ...
    const top = 20 + (i % 2) * 28;  // two lanes
    el.style.cssText = `
      animation-duration: ${dur}s;
      animation-delay: ${delay}s;
      top: ${top}px;
      background: ${type === 'truck' ? '#f59e0b' : type === 'two-w' ? '#a855f7' : accentColor};
    `;
...
/* ================================================================
   HOOK INTO EXISTING app.js  result handler
   Find your existing result display function and add these 2 lines:
